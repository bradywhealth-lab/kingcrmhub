    ...
    for dir in "$REPO_DIR"/prisma/migrations/*/; do
      ...
      case "$name" in
        ...
        *add_stripe_billing*)
          ...
          ;;
        *add_claim_grant*)
          CHECK_SQL="DO \$\$ BEGIN IF (SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ClaimGrant') < 1 THEN RAISE EXCEPTION 'ClaimGrant table missing'; END IF; IF (SELECT count(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'ClaimGrant' AND column_name IN ('licenseKeyHash', 'orderEmail', 'productId', 'source', 'organizationId', 'plan', 'grantedAt', 'expiresAt', 'redeemedAt', 'createdAt')) < 10 THEN RAISE EXCEPTION 'ClaimGrant columns missing (need all 10)'; END IF; IF (SELECT count(*) FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'ClaimGrant' AND indexname IN ('ClaimGrant_licenseKeyHash_key', 'ClaimGrant_orderEmail_productId_key', 'ClaimGrant_organizationId_idx', 'ClaimGrant_expiresAt_idx')) < 4 THEN RAISE EXCEPTION 'ClaimGrant indexes missing (need all 4)'; END IF; IF (SELECT count(*) FROM information_schema.table_constraints WHERE table_schema = 'public' AND table_name = 'ClaimGrant' AND constraint_name IN ('ClaimGrant_pkey', 'ClaimGrant_organizationId_fkey')) < 2 THEN RAISE EXCEPTION 'ClaimGrant constraints missing (need primary key and foreign key)'; END IF; END \$\$;"
          ;;
        *)
...
if [[ "$LANDING_HTML" != *'data-deploy-marker="public-landing-v3"'* ]]; then
