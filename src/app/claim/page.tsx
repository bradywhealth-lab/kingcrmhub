...
// Public /claim page — pre-signup entry point for the Gumroad promo
// (t_55f06113). Buyer enters the email from their receipt + license key; the
// server verifies via the Gumroad License API (or manual allowlist) and parks
// a single-use pending claim. Signup then binds it: the new org starts with
// the 1-month Studio overlay. Signing in with an existing account applies the
// grant to the org immediately.
//
// Product law (Brady decision record 2026-09-21): honest "Studio free for 1
// month" copy only — no fake badges, no auto-renew claims, no guarantees.
// No card required, and when the month ends the org keeps its data on the
// free plan until the owner chooses to upgrade at $39/mo.
...
function ClaimPageInner() {
  ...
  const [claimState, setClaimState] = useState<{ status: string; expiresAt: string } | null>(null)
  ...
  const submitClaim = async () => {
    ...
    try {
      ...
      setClaimState({ status: data.status, expiresAt: data.expiresAt })
    } catch (err) {
  ...
  // A verified claim continues to signup (prefilled) when the user is new;
  // a signed-in claim (status granted) lands in the workspace immediately.
  useEffect(() => {
    if (!claimState) return
    if (claimState.status === 'granted') {
      router.push('/')
      router.refresh()
      return
    }
    router.push(`/auth?mode=signup&email=${encodeURIComponent(email.trim())}`)
  }, [claimState, email, router])
  ...
  const verified =
    claimState && claimState.status !== 'granted'
      ? { headline: 'License verified. Your Studio month starts when you finish signup.', detail: `Free Studio through ${new Date(claimState.expiresAt).toLocaleDateString([], { year: 'numeric', month: 'long', day: 'numeric' })}. Finish creating your account to activate it.` }
      : null
        ...
        {!claimState ? (
          ...
          <div className="mt-8 rounded-3xl border p-6" style={{ borderColor: `${TEAL}55`, background: `${TEAL}14` }}>
            <div className="flex items-center gap-3">
              ...
              <p className="text-lg font-semibold" style={{ color: PAPER }}>{verified?.headline ?? 'Studio is active on your account.'}</p>
            </div>
            <p className="mt-2 text-sm" style={{ color: 'rgba(244,240,230,0.66)' }}>
              {verified?.detail ?? 'Your 1-month Studio promo is applied. You are being redirected.'}
            </p>
          </div>
        )}
