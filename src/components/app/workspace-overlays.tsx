          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between rounded border border-[var(--ink-line)] bg-muted p-2">
                  <span className="text-xs text-muted-foreground">Follow links</span>
                  <Switch checked={scrapeForm.followLinks} onCheckedChange={(value) => setScrapeForm((current) => ({ ...current, followLinks: value }))} />
                </div>
                <div className="flex items-center justify-between rounded border border-[var(--ink-line)] bg-muted p-2">
                  <span className="text-xs text-muted-foreground">Use headless/JS</span>
                  <Switch checked={scrapeForm.useHeadless} onCheckedChange={(value) => setScrapeForm((current) => ({ ...current, useHeadless: value }))} />
                </div>
                <div className="flex items-center justify-between rounded border border-[var(--ink-line)] bg-muted p-2">
                  <span className="text-xs text-muted-foreground">Rotate user agent</span>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div>
                <Label className="text-muted-foreground">Delay between requests (ms)</Label>
                <Input
                  type="number"
                  min={0}
                  max={10000}
              ...
              <div className="flex items-end">
                <div className="flex w-full items-center justify-between rounded border border-[var(--ink-line)] bg-muted p-2">
                  <span className="text-xs text-muted-foreground">Enable proxy provider</span>
                  <Switch checked={scrapeForm.proxyEnabled} onCheckedChange={(value) => setScrapeForm((current) => ({ ...current, proxyEnabled: value }))} />
                </div>
              </div>
              <div>
                <Label className="text-muted-foreground">Proxy provider</Label>
            ...
            {scrapeForm.proxyEnabled && scrapeForm.proxyProvider === "proxy_template" && (
              <div>
                <Label className="text-muted-foreground">Proxy template URL</Label>
                <Input
                  className="mt-1 border-[var(--ink-line)] bg-muted"
                ...
                />
                <p className="mt-1 text-xs text-muted-foreground">Use <code>{"{url}"}</code> placeholder where the target URL should be injected.</p>
              </div>
            )}
            ...
            <Separator />
            <div>
              <p className="mb-2 text-sm font-medium text-foreground">Recent scrape jobs</p>
              <div className="max-h-40 space-y-2 overflow-auto">
                {scrapeJobs.length === 0 ? (
                  <p className="text-xs text-muted-foreground">No jobs yet.</p>
                ) : scrapeJobs.map((job) => (
                  <div key={job.id} className="flex items-center justify-between gap-3 rounded border border-[var(--ink-line)] bg-muted p-2">
                    <div className="min-w-0">
                      <p className="truncate text-xs text-foreground">{job.sourceUrl}</p>
                      <p className="text-[11px] text-muted-foreground">{new Date(job.createdAt).toLocaleString()}</p>
                    </div>
                    <Badge
                      variant="outline"
                      className={cn(
